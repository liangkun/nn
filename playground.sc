...
val size = 1000
val train = DoubleMoon.gen(size, 1)
...
val (iters, error) = Perceptron.train(weights, xs, ys, maxIters = 1000, errorCollector = Some(ecollector))
//Main.plotDoubleMoon(1000)
val fig = new Figure("Playground")
val plt = fig.subplot(1, 2, 0)
utils.plotBinaryDataPoints(train).foreach(s => plt += s)
plt += utils.plotLine(weights, -12.0f, 12.0f)
